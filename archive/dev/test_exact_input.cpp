/**
 * Test with exact working input from Python
 */
...
int main() {
    try {
        std::cout << "=== Testing with Exact Python Input ===" << std::endl;
        
        // Load the exact binary input that works in Python
        std::vector<float> model_input;
        {
            std::ifstream file("working_input.bin", std::ios::binary);
            if (!file.is_open()) {
                throw std::runtime_error("Cannot open working_input.bin");
            }
            ...
            model_input.resize(num_floats);
            
            file.read(reinterpret_cast<char*>(model_input.data()), file_size);
            std::cout << "Loaded " << num_floats << " floats from binary file" << std::endl;
        }
        
        // Shape is [1, 80, 874]
        std::vector<int64_t> input_shape = {1, 80, 874};
        ...
        // Initialize ONNX Runtime exactly as Python does
        ...
        std::vector<int64_t> length_data = {874};
        ...
        // Expected from Python: [1024 1024 1024 1024 1024 1024   15 1024   24 1024]
        std::cout << "Expected: [1024 1024 1024 1024 1024 1024   15 1024   24 1024]" << std::endl;
        
        // Compare first few logit values with Python
        std::cout << "\nFirst 5 logit values at position 0:" << std::endl;
        for (int i = 0; i < 5; i++) {
            std::cout << "  Token " << i << ": " << output_data[i] << std::endl;
        }
